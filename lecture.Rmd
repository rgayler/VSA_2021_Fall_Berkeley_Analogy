...
structural similarity = literal similarity | <font size="5">$\{sun \mapsto nucleus, planet \mapsto electron\}$</font>
...
-   Distributive parallelism
    -   (A + B + C) \* $\rho$(P + Q + R) = A\*$\rho$P + A\*$\rho$Q + ... B\*$\rho$P + B\*$\rho$Q + ...
...
Focus of Chapter 6 of Plate's thesis (1994) is the use of
...
$\dagger$ This is equivalent to applying the substitution $Y \mapsto \textbf{1}$
...
-   Something needs to be done to map relational
    structure into the local space so that it can engage the similarity dynamics
...
![<font size="1">Mikolov et al (2013)</font>](assets/mikolov-offsets.png){width="70%"}
...
## Kanerva (2010) - Hand crafted substitution
